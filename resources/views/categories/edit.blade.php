...
@section('content')
    <x-page-header title="Edit Kategori" description="{{ $itemCategory->code }} — {{ $itemCategory->name }}" :breadcrumb="['Master Sistem', 'Kategori', 'Edit']">
        <x-button href="{{ route('categories.index') }}" variant="secondary"><i class="bi bi-arrow-left"></i> Kembali</x-button>
    </x-page-header>
    ...
    <form method="POST" action="{{ route('categories.update', $itemCategory) }}" x-data="{ submitting: false }" @submit="submitting = true">
        @csrf
        @method('PUT')
