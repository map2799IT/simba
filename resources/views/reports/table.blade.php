            ...
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @elseif ($reportType === 'stock_movements')
                        ...
                        <tr class="hover:bg-slate-50"><td class="whitespace-nowrap px-4 py-3 text-sm text-slate-600">{{ $mvD }}</td><td class="px-4 py-3 font-mono text-sm text-slate-600">{{ $mvC }}</td><td class="px-4 py-3 text-sm font-semibold text-slate-900">{{ $mvI }}</td><td class="px-4 py-3 text-sm text-slate-600">{{ $mvW }}</td><td class="px-4 py-3 text-sm text-slate-600">{{ $mvLabel }}</td><td class="px-4 py-3 text-sm text-slate-600">{{ $row->reference_number ?? '-' }}</td><td class="px-4 py-3 text-sm text-slate-600">{{ $row->source ?? $row->destination ?? '-' }}</td><td class="px-4 py-3 text-right text-sm text-slate-600">{{ \App\Support\QuantityFormatter::format($mvB) }}</td><td class="px-4 py-3 text-right text-sm font-bold {{ $mvDf>=0?'text-emerald-600':'text-red-600' }}">{{ $mvDf>=0?'+':'' }}{{ \App\Support\QuantityFormatter::format($mvDf) }}</td><td class="px-4 py-3 text-right text-sm font-bold text-slate-900">{{ \App\Support\QuantityFormatter::format($mvA) }}</td><td class="px-4 py-3 text-right text-sm text-slate-600">{{ $mvQ }}</td><td class="px-4 py-3 text-sm text-slate-600">{{ $mvU }}</td></tr>
                    @elseif ($reportType === 'loans')
